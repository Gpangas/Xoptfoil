...
! Data structures for airfoil optimization code
  ...
  type take_off_type
    ...
    integer :: niteration_weight, run_oppoint
  ...
  type climb_type
    ...
    double precision :: time, h, dh
    double precision, dimension(5) :: points_coeff
    double precision :: RC_max, V, Cl
    
  end type climb_type
  
  type cruise_type
    
    double precision :: time, h, dist_ref
...
! routines)
  ...
  double precision, dimension(max_addthickconst) :: addthick_x, addthick_min,  &
                                                    addthick_max
  double precision :: weight_min, climb_rate_min, cruise_speed_min, turn_speed_min
  ...
  double precision :: CD_ld, CD_add
  !add take_off data
  integer :: run_oppoint
  double precision :: h_take_off, A_1, miu, S_g
  double precision :: weight_empty, weight_payload_ref
  !add climb data
  double precision :: time_climb, h_climb, dh_climb
  double precision, dimension(5) :: points_coeff
  !add cruise data
  double precision :: time_cruise, h_cruise, dist_ref_cruise
  !add turn data
