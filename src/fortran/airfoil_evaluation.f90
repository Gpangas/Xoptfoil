...
! Analyze airfoil at requested operating conditions with Xfoil
  ...
  ! low rate of climb (23), upper cruise lift range (24), 
  ! below cruise lift range (25), low cruise speed (26), 
  ! low distance travelled (27), upper turn lift range (28), 
  ! below turn lift range (29), low turn speed (30)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
function calculate_objective_function(moment, drag, lift, alpha, viscrms, xtrt,&
  ...
  do i = 1, noppoint
    ...
    elseif (trim(optimization_type(i)) == 'take-off') then
      ...
      if((i == 1) .or. trim(optimization_type(i-1)) /= 'take-off')then  
        if (lift(i) > 0.d0) then
          increment = scale_factor(i)/lift(i)
        else
          increment = 1.D9   ! Big penalty for lift <= 0
  ...
  do i = 1, noppoint
    if((trim(optimization_type(i))=='take-off') .or. (trim(optimization_type(i)) &
      =='climb') .or. (trim(optimization_type(i))=='cruise'))then 
      calculate_objective_function = calculate_objective_function +              &
                                     3/(points(1)+points(2)+points(3))
      exit
    end if
  end do
...
! Analyze airfoil at requested operating conditions with Xfoil
  ...
  ! Set output file names and identifiers
    ...
    performancefile = trim(output_prefix)//'_design_flight_performance.dat'
  ...
  ! Write flight performance to file
    ...
    ntotal_points = 0
    if(ntake_off_constrain /= 0)then
      total_points = total_points + points(1)*1000
      ntotal_points = ntotal_points + 1 
    end if    
    if(nclimb_constrain /= 0)then
      total_points = total_points + points(2)*1000
      ntotal_points = ntotal_points + 1
    end if
    if(ncruise_constrain /= 0)then
      total_points = total_points + points(3)*1000
      ntotal_points = ntotal_points + 1
    end if    
    ...
    if(write_performance_file)then
      if(ntake_off_constrain /= 0)then
        ...
        take_off%V_to, weight, (weight - take_off%weight_empty),               & 
        (points(1)*1000), '|'
        1100 format(F14.8,F14.8,F14.8,F14.8,F14.8,A)
      end if
      if(nclimb_constrain /= 0)then
        write(performanceunit, 1200, advance='no') climb%RC_max, climb%V,      &
          climb%Cl, (points(2)*1000),'|'
        1200 format(F14.8,F14.8,F14.8,F14.8,A)
      end if
      if(nturn_constrain /= 0)then
      ...
      if(ncruise_constrain /= 0)then
        write(performanceunit, 1400, advance='no') cruise%V, cruise%dist,      &
          (points(3)*1000),'|'
        1400 format(F14.8,F14.8,F14.8,A)
      end if
