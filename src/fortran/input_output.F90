...
subroutine read_inputs(input_file, search_type, global_search, local_search,   &
  ...
  namelist /constraints/ min_thickness, max_thickness, moment_constraint_type, &
                         ...
                         climb_rate_min, cruise_speed_min, turn_speed_min
  ...
  namelist /aircraft_data/ weight_i, S_w, A_w, e_w, S_expose, thrust_coeff,    &
                           ...
                           t_skin_roughness, tail_surface_area, CD_add,        &
  ...
  namelist /take_off_data/ h_take_off, A_1, miu, S_g, weight_empty,            &
                           ...
                           weight_converge_limit, run_oppoint
  namelist /climb_data/ time_climb, h_climb, dh_climb, points_coeff
  namelist /cruise_data/ time_cruise, h_cruise, dist_ref_cruise
  namelist /turn_data/ activation_turn, h_turn, n_turn, field_length
...
! Set defaults for cruise_data namelist options        
  time_cruise = 60
  h_cruise = 300
...
!read aircraft_data, take_off_data, climb_data and cruise_data    
  ...
  rewind(iunit)
  read(iunit, iostat=iostat1, nml=cruise_data)
  call namelist_check('cruise_data', iostat1, 'warn')
  rewind(iunit)
...
!take_off_data in derived type
  ...
  take_off%run_oppoint = run_oppoint
...
!cruise_data in derived type
  cruise%time = time_cruise
  cruise%h = h_cruise
  cruise%dist_ref = dist_ref_cruise
  
!turn_data in derived type 
  turn%activation = activation_turn
  turn%h = h_turn
  turn%n = n_turn
  turn%field_length = field_length 
...
! Identification of performance operating points
  ...
  do i = 1, noppoint
    if(trim(optimization_type(i)) == 'take-off')then
      nflight_constrain = 1
      ntake_off_constrain = 2
    end if
    if(trim(optimization_type(i)) == 'climb')then
      nflight_constrain = 1
      nclimb_constrain = 1
    end if
    if(trim(optimization_type(i)) == 'cruise')then
      nflight_constrain = 1
      ncruise_constrain = 4
    end if
    if(trim(optimization_type(i)) == 'turn')then
      nflight_constrain = 1
      nturn_constrain = 3
    end if
  end do
  ...
  if (.not. match_foils) then
    contrain_number = 1+nflap_optimize+3+naddthickconst+10+                    &
                        ...
                        nflight_constrain+ntake_off_constrain+nclimb_constrain+&
                        ncruise_constrain+nturn_constrain
  else
    contrain_number = 1
  end if
...
! Constraints namelist
  ...
  write(*,*) " climb_rate_min = ", climb_rate_min
  write(*,*) " cruise_speed_min = ", cruise_speed_min
  write(*,*) " turn_speed_min = ", turn_speed_min
  write(*,'(A)') " /"
  write(*,*)
...
! Cruise_data namelist
   
  write(*,'(A)') "&cruise_data"
  write(*,*) " time = ", cruise%time
  write(*,*) " h = ", cruise%h
  write(*,*) " dist_ref = ", cruise%dist_ref
  write(*,'(A)') "/"
  write(*,*)
...
! Turn_data namelist
  ...
  write(*,*) " h = ", turn%h
  write(*,*) " n = ", turn%n
  write(*,*) " field_length = ", turn%field_length
  write(*,'(A)') "/"
  write(*,*)
...
! Constraints namelist
  ...
  write(100,*) " climb_rate_min = ", climb_rate_min
  write(100,*) " cruise_speed_min = ", cruise_speed_min
  write(100,*) " turn_speed_min = ", turn_speed_min
  write(100,'(A)') "/"
  write(100,*)
...
! Cruise_data namelist
  ...
  write(100,'(A)') "&cruise_data"
  write(100,*) " time = ", cruise%time
  write(100,*) " h = ", cruise%h
  write(100,*) " dist_ref = ", cruise%dist_ref
  write(100,'(A)') "/"
  write(100,*)
...
! Turn_data namelist
  ...
  write(100,*) " h = ", turn%h
  write(100,*) " n = ", turn%n
  write(100,*) " field_length = ", turn%field_length
  write(100,'(A)') "/"
  write(100,*)
! naca_airfoil namelist
...
! Operating points
  ...
  do i = 1, noppoint
    ...
    if (trim(optimization_type(i)) /= 'min-drag' .and.                         &
      ...
      trim(optimization_type(i)) /= 'cruise'.and.                              &
