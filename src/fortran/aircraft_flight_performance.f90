...
subroutine take_off_evaluation(oppoint_init, oppoint_end, drag, lift, points,  & 
  ...
  weight_loop: do j = 1, take_off%niteration_weight
    ...
    constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+&  
                      nlift_constrain+ndrag_constrain+1) = 0.d0
    if((penaltyval > epsexit) .and. penalize) then
      i_lift_unconverge = oppoint_init - 1 + i
      write(text,'(F9.6)') dble(i_lift_unconverge)
      constrains_vector(1+nflap_optimize+3+naddthickconst+10+                &  
                        nmoment_constrain+nlift_constrain+ndrag_constrain+1) &   
                        = dble(i_lift_unconverge)
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+&  
                        nlift_constrain+ndrag_constrain+2) = weight_converge
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                       nlift_constrain+ndrag_constrain+3) = weight
  ...
  points = weight_payload/take_off%weight_payload_ref
...
subroutine climb_evaluation(oppoint_init, oppoint_end, drag, lift, points,     & 
  ...
  use vardef, only : weight, climb, cruise, objfunction_type, nflap_optimize,  &
                     naddthickconst, nmoment_constrain, nlift_constrain,       & 
                     ndrag_constrain, ntake_off_constrain, climb_rate_min
  ...
  penaltyval = penaltyval + max(0.d0,climb_rate_min-climb%RC_max)/             & 
                 (climb_rate_min+1.0E-12)
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                      nlift_constrain+ndrag_constrain+1+ntake_off_constrain+1) &
                      = climb%RC_max
  ...
  h = climb%time*climb%RC_max
  
  if(h .lt. climb%dh)then
...
subroutine cruise_evaluation(oppoint_init_cruise, oppoint_end_cruise,          & 
  ...
  use vardef, only : weight, cruise, turn, nflap_optimize, naddthickconst,     &
                     ...
                     cruise_speed_min, turn_speed_min, objfunction_type
  ...
  double precision :: turn_perimeter, cruise_lenght
  double precision :: lift_converge, penaltyval
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                      nlift_constrain+ndrag_constrain+1+ntake_off_constrain+   &
                      nclimb_constrain+nturn_constrain+1) = T_cruise(1)-       & 
                      D_cruise(1)
  ...
  if((penaltyval > epsexit) .and. penalize) then
    ...
    cruise_penalty_return%value = penaltyval*1.0D+06
    cruise_penalty_return%message_code = 24
  ...
  accel_to_cruise: do i = 2, n_oppoint_cruise
    if((T_cruise(i)-D_cruise(i)) .gt. 0)then
      ...
      cruise%V = V_cruise(i)-(T_cruise(i)-D_cruise(i))/(((T_cruise(i)-         &
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                      nlift_constrain+ndrag_constrain+1+ntake_off_constrain+   &
                      nclimb_constrain+nturn_constrain+2) =                    & 
                      T_cruise(n_oppoint_cruise)-D_cruise(n_oppoint_cruise)
  ...
  penaltyval = penaltyval + max(0.d0,cruise_speed_min-cruise%V)/               & 
                 (cruise_speed_min+1.0E-12)
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                      nlift_constrain+ndrag_constrain+1+ntake_off_constrain+   &
                      nclimb_constrain+nturn_constrain+3) = cruise%V

  if ( (penaltyval > epsexit) .and. penalize ) then
    write(text,'(F9.6)') cruise%V
    cruise_penalty_return%value = penaltyval*1.0D+06
  ...
  if(turn%activation)then
    ...
    constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+  &  
                       ...
                       nclimb_constrain+1+1) = T_turn(1)-D_turn(1)
    ...
    constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+  &  
                       nlift_constrain+ndrag_constrain+ntake_off_constrain+    &
                       nclimb_constrain+1+2) = T_turn(n_oppoint_turn)-         & 
                       D_turn(n_oppoint_turn)
    ...
    if((penaltyval > epsexit) .and. penalize) then
      ...
      cruise_penalty_return%message = ' failed, turn lift coefficient below'// &
      ' the range. thurst minus drag: '// trim(text)//' at lift: '//trim(text_1) 
      cruise_penalty_return%constrains_data = constrains_vector
      return
    end if
    ...
    penaltyval = penaltyval + max(0.d0,turn_speed_min-turn%V)/                 & 
                   (turn_speed_min+1.0E-12)
    constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+  &  
                        nlift_constrain+ndrag_constrain+1+ntake_off_constrain+   &
                        nclimb_constrain+3) = turn%V
    ...
    cruise%dist = (cruise_lenght + turn_perimeter)/(cruise_lenght/cruise%V & 
      + turn_perimeter/turn%V) * cruise%time
  else
    !Calculate the distance travelled without turn ponderation
    cruise%dist = cruise%time*cruise%V
  end if
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                    nlift_constrain+ndrag_constrain+1+ntake_off_constrain+     &
                    nclimb_constrain+nturn_constrain+4)= cruise%dist
  ...
  points = cruise%dist/cruise%dist_ref
