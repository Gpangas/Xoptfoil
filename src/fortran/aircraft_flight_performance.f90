...
subroutine climb_evaluation(oppoint_init, oppoint_end, drag, lift, points,     & 
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                      nlift_constrain+ndrag_constrain+1+ntake_off_constrain+2) &
                      = climb%t_accel
...
subroutine cruise_evaluation(oppoint_init_cruise, oppoint_end_cruise,          & 
  ...
  accel_to_cruise: do i = 2, n_oppoint_cruise
    if((T_cruise(i)-D_cruise(i)) .gt. 0)then
      ...
      t_accel_i = (cruise%V-V_cruise(i-1))/((T_cruise(i-1)-D_cruise(i-1))      &
        /2*(g/weight))
      cruise%t_accel = cruise%t_accel + t_accel_i
      if(cruise%t_accel .gt. cruise%t_ex)then
        if(cruise%t_ex .gt. (cruise%t_accel-t_accel_i))then
          cruise%dist_accel = cruise%dist_accel + (cruise%V + V_cruise(i-1))/2 & 
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                    nlift_constrain+ndrag_constrain+1+ntake_off_constrain+     &
                    nclimb_constrain+nturn_constrain+4)= cruise%t_accel
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                    nlift_constrain+ndrag_constrain+1+ntake_off_constrain+     &
                    nclimb_constrain+nturn_constrain+5)= cruise%dist_accel
  ...
  if(turn%activation)then
    ...
    constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+  &  
                       nlift_constrain+ndrag_constrain+1+ntake_off_constrain+  &
                       nclimb_constrain+1) = T_turn(1)-D_turn(1)
    ...
    constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+  &  
                       nlift_constrain+ndrag_constrain+1+ntake_off_constrain+  &
                       nclimb_constrain+2) = (T_turn(n_oppoint_turn)-          &         
    ...
    if(cruise%accel)then
      if(cruise%t_accel .le. cruise%t_ex)then
        cruise%dist = (cruise_lenght + turn_perimeter)/(cruise_lenght/         & 
          cruise%V + turn_perimeter/turn%V) * (cruise%time) 
      else
        cruise%dist = (cruise_lenght + turn_perimeter)/(cruise_lenght/         & 
          cruise%V + turn_perimeter/turn%V) * (cruise%time -                   & 
    ...
    if(cruise%accel)then
      if(cruise%t_accel .le. cruise%t_ex)then
        ...
        cruise%dist = cruise%dist_accel + cruise%V*(cruise%time-               & 
          (cruise%t_accel - cruise%t_ex))
      end if  
    else
      cruise%dist = cruise%time*cruise%V
  ...
  constrains_vector(1+nflap_optimize+3+naddthickconst+10+nmoment_constrain+    &  
                    nlift_constrain+ndrag_constrain+1+ntake_off_constrain+     &
                    nclimb_constrain+nturn_constrain+6)= cruise%dist
